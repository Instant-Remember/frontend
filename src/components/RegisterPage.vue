    ...
    <section class="container">
      <div class="steps">
        ...
        <div v-if="step === 1" class="step1">
          <input type="text" id="first_name" v-model="userData.first_name" required placeholder="Имя">
          <input type="text" id="last_name" v-model="userData.last_name" required placeholder="Фамилия">
          <div class="circles">
        ...
        <div v-else-if="step === 2" class="step2">
          <input type="text" id="username" v-model="userData.username" required placeholder="Имя пользователя">
          <input type="email" id="email" v-model="userData.email" required placeholder="Почта">
          <div class="circles">
        ...
        <div v-else class="step3">
          <input type="password" id="password" v-model="userData.password" required placeholder="Пароль">
...
export default {
  ...
  methods: {
    ...
    signup() {
      ...
      const url = 'http://158.160.27.89:8000/signup';

      // Отправляем POST-запрос
      axios.post(url, this.userData)
        .then(response => {
          // Обработка успешного ответа от сервера
          console.log('Успешная регистрация:', response.userData);
        })
        .catch(error => {
          // Обработка ошибки
